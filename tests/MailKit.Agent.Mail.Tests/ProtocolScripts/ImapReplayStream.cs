...
internal sealed class ImapReplayCommand
{
    private static readonly Encoding Latin1 = Encoding.GetEncoding(28591);

    public ImapReplayCommand(string command, string response)
    {
        Command = command;
...
internal sealed class ImapReplayStream : Stream
{
    ...
    private readonly IReadOnlyList<ImapReplayCommand> commands;
    ...
    public ImapReplayStream(IReadOnlyList<ImapReplayCommand> commands)
    {
    ...
    public override int Read(byte[] buffer, int offset, int count)
    {
        ...
        if (response.Position == response.Length)
        {
    ...
    public override void Write(byte[] buffer, int offset, int count)
    {
        ...
        sent.Write(buffer, offset, count);
        ImapReplayCommand expected = commands[index];
    ...
    private static MemoryStream OpenResponse(ImapReplayCommand command) =>
        new(command.Response, writable: false);

    private void EnsureResponseReady()
    {
        if (state == ReplayState.SendResponse)
            return;
