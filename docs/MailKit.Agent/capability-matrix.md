...
| 发送 | 发送（两阶段确认，`send_mode` 二选一） | `send_prepare` + `send_commit` | `SmtpClient.Send`（`confirm_dialog`）；`ImapFolder.Append` 到草稿箱带 `\Draft`（`drafts`） | `confirm_dialog`：SMTP 连接、显式用户确认、一次性令牌、本机人工批准弹窗（人工拒绝返回 `send.approval_declined`，无交互桌面返回 `send.approval_unavailable`，均不消耗令牌）。`drafts`：IMAP 连接与可解析的草稿文件夹（缺失返回 `drafts.folder_not_found`），不投递、不弹批准框，用户在邮件客户端自行审阅、修改与发送 | 外部影响或不可逆 | `SendToolsTests.PrepareBindsProcessSessionAndReturnsRedactedPreviewWithToken`; `SendToolsTests.CommitConsumesOneTimeTokenAndDeliversExactlyOnce`; `SendApplicationTests.DeclinedApprovalDoesNotConsumeTokenOrSendOrWriteLedger`; `SendApplicationTests.DraftsCommitSkipsApprovalSavesDraftWithoutSmtp`; `SendApplicationTests.IndeterminateTransportOutcomeIsTerminalAndNeverResent`; `ImapDraftStoreTests.SpecialUseDraftsFolderAppendsWithDraftFlagAndExactLiteral` | 已支持 |
...
| 邮件 | 草稿管理（查看、编辑、删除既有草稿） | 无对应工具 | 不调用 | IMAP 连接及服务器所需能力 | 可恢复写入 | 未实现 | 计划中 — 后续计划 3：**可恢复写入和草稿** |
...
发送约束：发送的收件地址仅支持 ASCII（显示名支持 Unicode）；SMTPUTF8 服务器能力协商已实现，但当前经 MCP 不可达，属后续计划。发送提交阶段由服务器按账户 `send_mode` 分流：`confirm_dialog` 模式强制本机人工批准弹窗（人工拒绝返回 `send.approval_declined`；非 Windows 或无交互桌面快速返回 `send.approval_unavailable`，提示需在交互会话中重试；两种情况均不消耗确认令牌）；`drafts` 模式不投递、不弹批准框，提交只把带 `\Draft` 标记的邮件 IMAP APPEND 到该账户的草稿文件夹（优先 SPECIAL-USE `\Drafts`，否则按名称回退查找 "Drafts"/"草稿箱"；找不到返回 `drafts.folder_not_found`），保存成功后账本终态为 `drafted`，发送与否由用户在邮件客户端自行决定。
